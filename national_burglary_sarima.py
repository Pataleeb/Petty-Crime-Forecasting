...
file_path = "combined_Burglary.csv"
df = pd.read_csv(file_path)

print(df.head())
df_us = df[df["series"] == "United States Total"].drop(columns=["series"])

# Print column names to verify their format
#print("Column Names Before Transpose:", df_us.columns.tolist())
...
df_us.index = pd.to_datetime(df_us.index, format='%m-%Y', errors='coerce')
...
plt.plot(df_us.index, df_us["Burglary_Cases"], label="United States Burglary Cases", color="blue")
plt.title("United States Total Burglary Cases Over Time")
plt.xlabel("Date")
...
check_stationarity(df_us["Burglary_Cases"])
df_us["Log_Burglary"] = np.log1p(df_us["Burglary_Cases"])
df_us["Log_Diff1"] = df_us["Log_Burglary"].diff()
df_us = df_us.dropna()  # Drop NaN values caused by differencing
...
plt.xlabel("Date")
plt.ylabel("Differenced Burglary Cases")
plt.legend()
plt.grid()
#plt.show()
###SARIMA model
...
last_log_value = df_us.loc[last_valid_date, "Log_Burglary"]
print("Available columns in df_us:", df_us.columns.tolist())
...
test = df_us.loc["2023-01-01":"2023-12-01", "Burglary_Cases"]  # Actual data for 2023

# Define and fit different SARIMA models
#model_1 = SARIMAX(df_us["Burglary_Cases"], order=(1,1,1), seasonal_order=(1,1,1,12)).fit()
#model_2 = SARIMAX(df_us["Burglary_Cases"], order=(2,1,2), seasonal_order=(1,1,1,12)).fit()
#model_3 = SARIMAX(df_us["Burglary_Cases"], order=(1,1,2), seasonal_order=(1,1,1,12)).fit()

# Compare AIC/BIC values
#print(f"Model 1 AIC: {model_1.aic}, BIC: {model_1.bic}")
#print(f"Model 2 AIC: {model_2.aic}, BIC: {model_2.bic}")
#print(f"Model 3 AIC: {model_3.aic}, BIC: {model_3.bic}")
...
forecast_diff = sarima_model.forecast(steps=12)

# Reverse Differencing (Cumulative Sum)
last_log_value = df_us.loc["2022-12-01", "Log_Burglary"]
...
# Ensure values are valid (no negative values)
forecast_actual[forecast_actual < 0] = 0

# Create Forecast DataFrame
forecast_dates = pd.date_range(start="2023-01-01", periods=12, freq="MS")
forecast_df = pd.DataFrame({"Forecasted Burglary Cases": forecast_actual}, index=forecast_dates)
print(df_us.columns)

# Extract actual burglary cases for 2023
actual_values = df_us.loc["2023-01-01":"2023-12-01", "Burglary_Cases"]

# Combine actual and forecasted values into a single DataFrame
results_df = pd.DataFrame({
    "Actual Burglary Cases": actual_values,
    "Forecasted Burglary Cases": forecast_actual.values
}, index=forecast_dates)

# Print the results
print("\nSARIMA Forecast vs Actual Burglary Cases (2023):")
print(results_df)

plt.figure(figsize=(12, 5))

# Plot Actual Values (2023)
plt.plot(test.index, test, label="Actual Burglary Cases (2023)", color="black", marker="o")

# Plot Predicted Values (2023)
plt.plot(forecast_df.index, forecast_df["Forecasted Burglary Cases"],
         label="SARIMA Predicted Cases (2023)", color="red", linestyle="dashed", marker="s")


plt.title("Actual vs Predicted Burglary Cases (2023)")
plt.xlabel("Date")
plt.ylabel("Number of Cases")
plt.legend()
plt.grid()
...
if len(test_holiday) != len(forecast_holiday):
    ...
    plt.plot(test_holiday.index, test_holiday, label="Actual Burglary Cases (Holiday Months)",
             color="black", marker="o")

    # Plot Predicted Holiday Values
    plt.plot(forecast_holiday.index, forecast_holiday["Forecasted Burglary Cases"],
             label="SARIMA Predicted Cases (Holiday Months)", color="red", linestyle="dashed", marker="s")


    plt.title("Actual vs Predicted Burglary Cases (Holiday Months: Jan, Jul, Oct, Nov, Dec)")
    plt.xlabel("Date")
    plt.ylabel("Number of Cases")
    plt.legend()
    plt.grid()
...
forecast_2023 = sarima_model.get_forecast(steps=12)
...
test_df = test.to_frame().rename(columns={"Burglary_Cases": "Actual Cases"})
...
plt.title("Anomaly Detection: Holiday Month Burglary Cases (2023)")
plt.xlabel("Date")
plt.ylabel("Number of Cases")
plt.legend()
plt.grid()
